...
using Xc.DataObjects.Plug_In.Tree;
...
namespace Xc.Application
{
    public class ModuleAppService : ApplicationService
    {
        ...
        //{
        //    return OperationBaseService.GetPageResultToJqGrid<ModuleDataObject, Module, IModuleRepository>(_moduleRepository, pageIndex, pageSize, t => t.Id);
        //}
        ...
        public JqGrid GetAllModule(int pageIndex, int pageSize)
        {
            ...
            //先查询出所有一级模块
            var firstModule = moduledtos.Where(t => !t.ParentId.HasValue || t.ParentId.Value == Guid.Empty).ToList();
            foreach (var item in firstModule)
            {
                item.level = 0; item.parent = null; item.expanded = false; item.isLeaf = true;
                item.loaded = true;
        ...
        public List<ZTreeData> GetAllModuleForZTree()
        {
            ...
            var modules = _moduleRepository.GetAll().ToList();
            var moduledtos = OperationBaseService.GetMapperData<List<Module>, List<ModuleDataObject>>(modules).OrderBy(t => t.Sort).ToList();
            //先查询出所有一级模块
            var firstModule = moduledtos.Where(t => !t.ParentId.HasValue || t.ParentId.Value == Guid.Empty).OrderBy(t => t.Sort).ToList();
            foreach (var item in firstModule)
            {
        ...
        /// <returns></returns>
        private OperationResult OperationRole(ModuleDataObject entity, OperationType operationType)
        {
